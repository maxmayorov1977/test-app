                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @if (count($errors) > 0)
                            ...
                            @endforeach
                        @endif
                        <form method="POST" action="/carUpdate/<?php echo $car['id']; ?>" enctype="multipart/form-data" spellcheck="false">
                            @csrf
                            ...
                            <br/>
                            <div class="md-form">
                                <input type="text" name="year_release" class="form-control" value="{{ $car['year_release'] }}">
                                <label for="year_release">{{ __('Год выпуска') }}</label>
                            </div>
                            <br/>
                            <div class="md-form">
                                <input type="text" name="horse_power" class="form-control" value="{{ $car['horse_power'] }}">
                                ...
                                <select name="transmission_id" class="custom-select custom-select-sm">
                                    ...
                                    @foreach ($transmissions as $transmission)
                                        @if($transmission['id'] == $car['transmission_id'])
                                            <option selected value="{{ $transmission['id'] }}">{{ $transmission['type'] }}</option>
                                        @else
